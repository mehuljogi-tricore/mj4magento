...
class Envato_Wishlist_IndexController extends Mage_Wishlist_IndexController
{
    /**
    ...
    public function preDispatch()
    {
        ...
        if (!Mage::getStoreConfigFlag('wishlist/general/active')) {
			...
			$response['url'] = Mage::getSingleton('customer/session')->getBeforeWishlistUrl();
    ...
    private function toOutput($status, $message, $count){
    ...
    protected function _addItemToWishList()
    {
        ...
        $success_msg = "Success";
        $fail_msg = "Failed";
        $wishHelper = Mage::helper('wishlist');
        $count = $wishHelper->getItemCount();
        
        $wishlist = $this->_getWishlist();
        if (!$wishlist) {
			$this->toOutput(false, "Failed", $count);
			//return $this->norouteAction();
			return;
        }
        ...
        if (!$productId) {
			$this->toOutput(false, "Product not found", $count);
            //$this->_redirect('*/');
            return;
        }
        ...
        if (!$product->getId() || !$product->isVisibleInCatalog()) {
            ...
            //$this->_redirect('*/');
            $this->toOutput(false, "Cannot specify product.", $count);
            return;
        }
        ...
        try {
			...
			$this->toOutput(false, "Successfully Added.", $this->__('An error occurred while adding item to wishlist: %s', $e->getMessage()), $count);
            //$session->addError($this->__('An error occurred while adding item to wishlist: %s', $e->getMessage()));
        }
        catch (Exception $e) {
			$this->toOutput(false, "Successfully Added.", $this->__('An error occurred while adding item to wishlist.'), $count);
            //$session->addError($this->__('An error occurred while adding item to wishlist.'));
        }
		$count = $wishHelper->getItemCount();
		$this->toOutput(true, $product->getName() . " is successfully added.", $count);
