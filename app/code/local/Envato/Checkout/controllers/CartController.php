...
class Envato_Checkout_CartController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    public function addAction()
    {
		...
		if (!$this->_validateFormKey()) {
			...
			try {
				...
				if (!$product) {
					//$this->_goBack();
					$fail_msg = "Product not available";
					$status = false;
					return;
				}
		...
		$response['count'] = ($status)?Mage::helper('checkout/cart')->getSummaryCount():$count;
		$response['message'] = ($status)?$success_msg:$fail_msg;
